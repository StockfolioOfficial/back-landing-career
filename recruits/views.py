...
class RecruitListAdminView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_id = "(관리자 전용) 채용공고 목록 조회 (직무별 / 최신순)",
...
#(관리자) 내가 올린 채용공고 목록 조회
class RecruitMyListAdminView(APIView):
    parameter_token = openapi.Parameter (
                                        "Authorization", 
                                        openapi.IN_HEADER, 
                                        description = "access_token", 
...
#(관리자) 공고 상세 조회
class RecruitDetailAdminView(APIView):
    parameter_token = openapi.Parameter (
                                        "Authorization", 
                                        openapi.IN_HEADER, 
                                        description = "access_token", 
...
#(관리자) 공고 생성
class RecruitCreateView(APIView):
    parameter_token = openapi.Parameter (
                                        "Authorization", 
                                        openapi.IN_HEADER, 
                                        description = "access_token", 
    ...
    @swagger_auto_schema (
        ...
        operation_description = "포지션, 설명, 기술스택, 근무타입(정규직/계약직), 경력타입(신입or경력or신입/경력), 채용인원, 모집마감일을 body에 담아 보내주세요."
    )
    @admin_only
...
#(관리자) 공고 수정 및 삭제
class RecruitModifyView(APIView):
    parameter_token = openapi.Parameter (
                                        "Authorization", 
                                        openapi.IN_HEADER, 
                                        description = "access_token", 
    ...
    def patch(self, request, recruit_id):
        try:
            ...
            career_type    = data["career_type"] if data["career_type"] else "신입/경력"
            deadline       = data["deadline"]
            author         = request.user.name if request.user.name else request.user.email
